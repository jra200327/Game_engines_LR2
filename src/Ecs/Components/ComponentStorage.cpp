...
template <typename T>
T& ComponentStorage<T>::Add(const int entityIid, const T& value)
{
    Resize((entityIid / 64 + 1) * 64, _data.size() == _count + 1 ? _data.size() + 64 :
    _data.size());
    _data[_count] = value;
...
template <typename T>
bool ComponentStorage<T>::Has(const int entityIid) const
{ 
...
template <typename T>
const std::span<const T> ComponentStorage<T>::All() const 
{ 
    return std::span(_data.begin(), _count); 
}

template <typename T>
const std::span<const int> ComponentStorage<T>::Entities() const 
{ 
